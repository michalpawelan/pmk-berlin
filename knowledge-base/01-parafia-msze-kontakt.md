# Parafia PMK Berlin — adres, biuro, Msze Święte / Pfarrei — Adresse, Büro, Messzeiten

Źródło: oficjalna strona pmk-berlin.de oraz wewnętrzne ustalenia z rektoratem (kwiecień 2026).
Quelle: offizielle Webseite pmk-berlin.de sowie interne Festlegungen mit dem Rektorat (April 2026).

---

## PL — Dane parafii

**Polska Misja Katolicka w Berlinie** — duszpasterstwo polskie przy Archidiecezji Berlin, działa od 1982 roku. Prowadzona przez Księży Salezjanów (SDB).

Kościół główny: **Johannes-Basilika**, Lilienthalstraße 5, 10965 Berlin-Neukölln.
E-mail: pmk@pmk-berlin.de
Strona: pmk-berlin.de
Kanał WhatsApp (anonimowy, ogłoszenia parafialne): link znajduje się w stopce strony pmk-berlin.de.

### Biuro parafialne

- **Poniedziałek:** 10:00–13:00 oraz 15:00–17:30
- **Środa:** 10:00–13:00 oraz 15:00–17:30
- W pozostałe dni biuro jest zamknięte.
- **Nie trzeba się wcześniej umawiać** — wystarczy przyjść w godzinach otwarcia.
- Ksiądz **nie** jest dostępny w biurze na osobiste rozmowy. W sprawach pilnych można podejść do księdza bezpośrednio po dowolnej Mszy Świętej.

### Numer telefonu — ważna zasada

Na oficjalnej stronie widnieje numer +49 30 752 40 80. **Podczas rozmowy telefonicznej Marta NIE podaje numeru** — rozmówca już na niego zadzwonił i wylądowałby z powrotem na tej linii. W czacie internetowym numer można podać, jeśli ktoś go wyraźnie poprosi, ale zaleca się przekierować do biura przez e-mail lub osobiście.

**Jedyny wyjątek:** w nagłym wypadku medycznym powiedz, żeby zadzwonić pod **112** (numer alarmowy w Niemczech).

---

## PL — Msze Święte — Johannes-Basilika (kościół główny)

- **Niedziela i święta:** 10:15, 12:00, 18:00
- **Poniedziałek – Sobota:** 7:00 i 18:00

Uroczysta Msza niedzielna ze śpiewem — szczególnie o 12:00.

## PL — Msze Święte — pozostałe kościoły (filie)

- **St. Joseph, Wedding:** Müllerstraße 161, 13353 Berlin — niedziela/święta **16:00**
- **St. Marien am Behnitz, Spandau:** Flankenschanze 43, 13585 Berlin — niedziela/święta **12:00**, środa **18:30**
- **St. Marien, Karlshorst:** Gundelfinger Straße 36, 10318 Berlin — niedziela/święta **11:00**

## PL — Spowiedź Święta

- **Codziennie (poniedziałek–sobota): podczas wieczornej Mszy Świętej o 18:00** w Bazylice.
- **Niedziela:** przed i w trakcie każdej Mszy Świętej.
- **Pierwszy piątek miesiąca:** spowiedź wcześniej, **od 17:00**.
- **Pozostałe piątki:** normalnie (podczas Mszy o 18:00) — żadnych dodatkowych godzin.
- Dodatkowe terminy w Adwencie i Wielkim Poście — ogłaszane w parafii.
- Spowiedź odbywa się po polsku.

## PL — Otwarcie Bazyliki / godziny otwarcia kościoła

Bazylika jest otwierana **30 minut przed każdym nabożeństwem** (Msza Święta, nieszpory, nabożeństwo). Poza godzinami nabożeństw kościół jest zamknięty.

**Zwiedzanie Bazyliki:** proszę o wcześniejszy kontakt z biurem parafialnym — telefonicznie lub przez e-mail pmk@pmk-berlin.de.

## PL — Nabożeństwa Wielkiego Postu

- **Droga Krzyżowa** — piątek o 17:30.
- **Gorzkie Żale** — niedziela o 17:30.

---

## DE — Angaben zur Pfarrei

**Polnische Katholische Mission in Berlin** — polnische Seelsorge im Erzbistum Berlin, seit 1982. Geführt von den Salesianern Don Boscos (SDB).

Hauptkirche: **Johannes-Basilika**, Lilienthalstraße 5, 10965 Berlin-Neukölln.
E-Mail: pmk@pmk-berlin.de
Website: pmk-berlin.de
WhatsApp-Kanal (anonym, Pfarrnachrichten): der Link steht im Footer der Website pmk-berlin.de.

### Büro

- **Montag:** 10:00–13:00 und 15:00–17:30
- **Mittwoch:** 10:00–13:00 und 15:00–17:30
- An den anderen Tagen ist das Büro geschlossen.
- **Kein Termin nötig** — einfach während der Öffnungszeiten vorbeikommen.
- Der Priester ist **nicht** für persönliche Gespräche im Büro erreichbar. In dringenden Fällen kann der Priester direkt nach einer Messe angesprochen werden.

### Telefonnummer — wichtige Regel

Auf der Website steht +49 30 752 40 80. **Im Telefongespräch nennt Marta die Nummer NICHT** — der Anrufer hat ja gerade diese Nummer gewählt. Im Web-Chat kann die Nummer genannt werden, wenn ausdrücklich danach gefragt wird; besser ist es, auf das Büro per E-Mail oder persönlich zu verweisen.

**Einzige Ausnahme:** In einem medizinischen Notfall den Anrufer bitten, sofort die **112** (europäische Notrufnummer) zu wählen.

---

## DE — Messzeiten — Johannes-Basilika (Hauptkirche)

- **Sonntag und Feiertage:** 10:15, 12:00, 18:00
- **Montag – Samstag:** 7:00 und 18:00

Die feierliche Sonntagsmesse mit Gesang ist besonders die 12-Uhr-Messe.

## DE — Messzeiten — weitere Kirchen

- **St. Joseph, Wedding:** Müllerstraße 161, 13353 Berlin — Sonntag/Feiertag **16:00**
- **St. Marien am Behnitz, Spandau:** Flankenschanze 43, 13585 Berlin — Sonntag/Feiertag **12:00**, Mittwoch **18:30**
- **St. Marien, Karlshorst:** Gundelfinger Straße 36, 10318 Berlin — Sonntag/Feiertag **11:00**

## DE — Beichte

- **Täglich (Montag–Samstag): während der Abendmesse um 18:00** in der Basilika.
- **Sonntag:** vor und während jeder Messe.
- **Erster Freitag im Monat:** Beichtgelegenheit bereits **ab 17:00**.
- **Andere Freitage:** normal (während der 18:00-Messe) — keine zusätzlichen Zeiten.
- Zusätzliche Termine im Advent und in der Fastenzeit — werden in der Pfarrei bekanntgegeben.
- Beichte findet auf Polnisch statt.

## DE — Öffnungszeiten der Basilika

Die Basilika wird **30 Minuten vor jedem Gottesdienst geöffnet** (Heilige Messe, Vesper, Andacht). Außerhalb der Gottesdienstzeiten ist die Kirche geschlossen.

**Kirchenbesichtigung:** bitte vorher das Pfarrbüro kontaktieren — telefonisch oder per E-Mail an pmk@pmk-berlin.de.

## DE — Andachten in der Fastenzeit

- **Kreuzweg (Droga Krzyżowa)** — Freitag um 17:30.
- **Gorzkie Żale (polnische Passionsgesänge)** — Sonntag um 17:30.

---

## Sprache / Język — wspólnota polskojęzyczna / polnischsprachige Gemeinde

**PL — Język wspólnoty**

Polska Misja Katolicka w Berlinie jest wspólnotą polskojęzyczną. Wszystkie sakramenty, grupy parafialne i nabożeństwa odbywają się po polsku. Osobom, które potrzebują posługi sakramentalnej po niemiecku lub angielsku, polecamy najbliższą niemiecką parafię.

Dotyczy to także:
- chrztu, pierwszej Komunii Świętej, bierzmowania, ślubu, namaszczenia chorych — wszystkie sakramenty udzielane są w języku polskim;
- katechezy, spotkań grup parafialnych, Drogi Krzyżowej, Gorzkich Żali, nieszporów — wyłącznie po polsku;
- spowiedzi — wyłącznie po polsku (jak opisano powyżej w sekcji o spowiedzi).

Jeżeli ktoś dzwoni lub pisze po niemiecku z prośbą o Mszę, chrzest, ślub lub inny sakrament po niemiecku — uprzejmie wyjaśnij, że PMK jest wspólnotą polskojęzyczną, i poleć kontakt z najbliższą niemiecką parafią Archidiecezji Berlin (erzbistumberlin.de).

**DE — Sprache der Gemeinde**

Die Polnische Katholische Mission Berlin ist eine polnischsprachige Gemeinde. Alle Sakramente, Pfarrgruppen und Andachten finden auf Polnisch statt. Wenn Sie auf Deutsch oder Englisch ein sakramentales Anliegen haben, verweisen wir Sie an die benachbarte deutsche Pfarrei.

Das gilt auch für:
- Taufe, Erstkommunion, Firmung, Eheschließung, Krankensalbung — alle Sakramente werden auf Polnisch gespendet;
- Katechese, Pfarrgruppen, Kreuzweg, Gorzkie Żale, Vesper — ausschließlich auf Polnisch;
- Beichte — ausschließlich auf Polnisch (siehe Abschnitt Beichte oben).

Wenn jemand auf Deutsch anruft oder schreibt und um eine Messe, Taufe, Trauung oder ein anderes Sakrament auf Deutsch bittet — bitte freundlich erklären, dass die PMK eine polnischsprachige Gemeinde ist, und auf die nächstgelegene deutsche Pfarrei des Erzbistums Berlin verweisen (erzbistumberlin.de).

---

## EN — Quick reference

- Main church: Johannes-Basilika, Lilienthalstraße 5, 10965 Berlin-Neukölln.
- Email: pmk@pmk-berlin.de
- Office: Monday and Wednesday only, 10:00–13:00 and 15:00–17:30. No appointment needed.
- Masses — Johannes-Basilika: Sunday 10:15, 12:00, 18:00. Monday through Saturday 7:00 and 18:00.
- Confession: during the daily 18:00 Mass (Mon–Sat); Sunday before and during every Mass; first Friday of the month from 17:00.
- Church opens 30 minutes before every service; closed outside service times. Visits/tours: contact the office first (phone or pmk@pmk-berlin.de).
- The priest does not hold personal appointments at the office. After any Mass is the usual way to speak with him.
- Emergencies: European emergency number 112.
